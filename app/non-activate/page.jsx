...
import { UserX, User, Mail, Phone, Hash, Calendar, ChevronRight, Search, RefreshCw } from 'lucide-react';
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: '13px', marginTop: '4px' }}>
            Investors who registered but have not yet completed profile activation
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button
            onClick={load}
            style={{ background: 'none', border: '1px solid var(--color-border)', borderRadius: '8px', color: 'var(--color-text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px', fontSize: '13px' }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 360px' : '1fr', gap: '20px', alignItems: 'start' }}>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          {loading ? (
            ...
            <div style={{ padding: '48px', textAlign: 'center', color: 'var(--color-text-muted)' }}>
              ...
              <p style={{ margin: 0 }}>{search ? 'No results found.' : 'No non-activated accounts.'}</p>
            </div>
          ) : (
              ...
              <tbody>
                {filtered.map(inv => (
                  ...
                  >
                    ...
                    <td style={{ color: 'var(--color-text-muted)', fontSize: '13px' }}>{inv.email || '—'}</td>
        ...
        {selected && (
          <div className="card" style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                ...
                <div key={label} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '11px', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{label}</div>
            ...
            <div style={{ marginTop: '20px', padding: '12px', borderRadius: '8px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', fontSize: '12px', color: '#EF4444' }}>
              This investor has not completed Step 2 (name, NID, photo). Their account is registered but not yet activated as an Investment Partner.
            </div>

            <button
              onClick={() => router.push(`/investor/${selected.id}`)}
